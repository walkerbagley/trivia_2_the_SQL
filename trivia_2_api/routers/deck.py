...
def add_round(cur: Cursor, deck_id: UUID, round: DeckRoundRequest, round_number:int | None = None) -> None:
    ...
    add_questions(cur, deck_id, round_id, round)
    return None
        

def add_questions(cur: Cursor, deck_id: UUID, round_id: UUID, round: DeckRoundRequest) -> None:
...
async def update_deck_round(round_id: UUID, round: DeckRoundRequest) -> None:
    with db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute('''UPDATE "DeckRounds" SET num_questions = %s, categories = %s, attributes = %s WHERE round_id = %s RETURNING deck_id, round_number''',
            ...
            cur.execute('''DELETE FROM "RoundQuestions" WHERE round_id = %s''', (round_id,))
