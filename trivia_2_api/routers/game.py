...
async def get_game() -> list[Game]:
    with db.connection() as conn:
        with conn.cursor(row_factory=class_row(Game)) as cur:
            cur.execute(
                '''SELECT id, deck_id, host_id, join_code, question_time_sec, status, current_round, current_question, start_time, end_time FROM "Games"'''
            )
            return cur.fetchall()
...
async def get_game(id: UUID) -> Game:
    with db.connection() as conn:
        with conn.cursor(row_factory=class_row(Game)) as cur:
            cur.execute(
                """SELECT id, deck_id, host_id, join_code, question_time_sec, status, current_round, current_question, start_time, end_time FROM "Games" WHERE id = %s""",
                (id,),
            )
...
async def create_game(request: Request, game: GameRequest) -> None:
    with db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            ...
            cur.execute(
                """INSERT INTO "Games" (deck_id, host_id, start_time, join_code, question_time_sec) VALUES (%s, %s, %s, %s, %s) RETURNING id""",
                (
                    game.deck_id,
                    request.state.user.id,
                    datetime.now(),
                    join_code,
                    game.question_time_sec,
                ),
            )
            ...
            return JSONResponse(
                status_code=201,
                content={"id": str(game_id), "join_code": str(join_code)},
            )
...
async def get_game_state(game_id: str, request: Request):
    ...
    with db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            ...
            cur.execute(
                """
                SELECT team_id FROM "GamePlayers" 
                WHERE game_id = %s AND player_id = %s AND is_active = true
            """,
                (game_id, user.id),
            )
            ...
            if not player:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail="Not in this game"
                )
            ...
            # Get current question_id from RoundQuestions
            cur.execute(
                """
                SELECT rq.question_id
                FROM "DeckRounds" dr
                JOIN "RoundQuestions" rq ON dr.round_id = rq.round_id
                JOIN "Games" g ON g.deck_id = dr.deck_id
                WHERE g.id = %s 
                  AND dr.round_number = %s
                  AND rq.question_number = %s
            """,
                (game_id, game["current_round"], game["current_question"]),
            )

            question_row = cur.fetchone()
            question_id = question_row["question_id"] if question_row else None

            # Get team's current answer if exists
            cur.execute(
                """
            ...
            """,
                (game_id, team_id, game["current_round"], game["current_question"]),
            )

            answer_row = cur.fetchone()
            team_answer = answer_row["answer"] if answer_row else None

            return {
                "status": game["status"],
                "current_round": game["current_round"],
                "current_question": game["current_question"],
                "question_time_sec": game["question_time_sec"],
                "last_question_start": game["last_question_start"],
...
async def join_game(request: Request, game: JoinGameRequest) -> None:
    with db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute(
                """SELECT id, host_id, status, join_code FROM "Games" WHERE join_code = %s and status = 'open' """,
                (game.join_code,),
            )
            ...
            cur.execute(
                """INSERT INTO "GamePlayers" (game_id, player_id, team_id, is_active) VALUES (%s, %s, %s, %s) ON CONFLICT DO NOTHING""",
                (results.get("id", None), request.state.user.id, game.team_id, True),
            )

            return JSONResponse(
                status_code=201, content={"game_id": str(results.get("id", None))}
            )
...
async def start_game(game_id: UUID) -> None:
    with db.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """UPDATE "Games" SET status = 'in_progress', last_question_start = now() WHERE id = %s""",
                (game_id,),
            )
...
async def leave_game(request: Request, game_id: UUID) -> None:
    with db.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """UPDATE "GamePlayers" SET is_active = false WHERE game_id = %s and player_id = %s""",
                (game_id, request.state.user.id),
            )
...
async def rejoin_game(request: Request) -> None:
    with db.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """UPDATE "GamePlayers" SET is_active = true WHERE game_id = 
...
async def end_game(request: Request, game_id: UUID) -> None:
    with db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute(
                """SELECT g.host_id FROM "Games" as g WHERE g.id = %s""", (game_id,)
            )
            results = cur.fetchone()
            ...
            cur.execute(
                """UPDATE "Games" SET status = 'complete', end_time = %s WHERE id = %s""",
...
#                         SELECT %s, gp.team_id, %s, %s, %s, %s
#                         FROM "GamePlayers" as gp
#                         WHERE gp.player_id = %s and gp.is_active = true and gp.game_id = %s
#                         ON CONFLICT (game_id, team_id, round_number, question_number) DO UPDATE SET answer = EXCLUDED.answer''', (game_id, answer.round_number, answer.question_number, question_id, answer.answer, request.state.user.id, game_id))
        ...
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute(
                """ 
                        ...
                        INNER JOIN "Decks" as d ON g.deck_id = d.id
                        INNER JOIN "DeckRounds" as dr ON d.id = dr.deck_id
                        WHERE g.id = %s and dr.round_number = g.current_round""",
                (game_id,),
            )
            game = cur.fetchone()
            ...
            if game.get("host_id", None) != request.state.user.id:
                raise HTTPException(
                    status_code=401, detail="Only host can move to next question"
                )

            if game.get("current_question", None) < game.get("num_questions", None):
                cur.execute(
                    """UPDATE "Games" SET current_question = current_question + 1, last_question_start = now() WHERE id = %s""",
                    (game_id,),
                )
                return
                ...
                cur.execute(
                    """ SELECT count(*) as rounds FROM "DeckRounds" as dr WHERE dr.deck_id = (SELECT g.deck_id FROM "Games" as g WHERE g.id = %s)""",
                    (game_id,),
                )
                ...
                if results.get("rounds", None) > game.get("current_round", None):
                    cur.execute(
                        """UPDATE "Games" SET current_round = current_round + 1, current_question = 1, last_question_start = now() WHERE id = %s""",
                        (game_id,),
                    )
                    return
