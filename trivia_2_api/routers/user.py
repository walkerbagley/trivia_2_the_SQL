    ...
    with db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            ...
            if status == "open":
                return UserStatus(
                    user_status=(
                        "hosting" if host_id == request.state.user.id else "playing"
                    ),
                ...
                cur.execute(
                    """
                            SELECT g.id, g.status, g.current_round as round_number, g.current_question as question_number, q.id as question_id, a.answer as team_answer, 
                            CASE WHEN g.status = 'in_progress'
                                THEN g.question_time_sec - EXTRACT(EPOCH FROM (now() - g.last_question_start))::int
                                ELSE null
                            END as time_remaining
...
async def get_user(id: UUID) -> UserResponse:
    ...
    with db.connection() as conn:
        with conn.cursor(row_factory=class_row(UserResponse)) as cur:
            cur.execute(
                """SELECT id, user_name, is_admin FROM "Users" WHERE id = %s""", (id,)
            )
            user = cur.fetchone()
            if user is None:
...
async def update_user(id: UUID, user: UserRequest) -> None:
    with db.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """UPDATE "Users" SET user_name = %s, hashed_password=%s WHERE id = %s""",
...
async def add_user_deck(user_id: UUID, deck_id: UUID) -> None:
    with db.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO "UserDecks" (user_id, deck_id) VALUES (%s, %s)""",
                (user_id, deck_id),
            )
...
async def remove_user_deck(user_id: UUID, deck_id: UUID) -> None:
    with db.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """DELETE FROM "UserDecks" WHERE user_id = %s AND deck_id = %s""",
                (user_id, deck_id),
            )
