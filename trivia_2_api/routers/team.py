...
async def get_team() -> list[Team]:
    with db.connection() as conn:
        with conn.cursor(row_factory=class_row(Team)) as cur:
            cur.execute('''SELECT t.id, t.name, ARRAY(SELECT user_id FROM "TeamMembers" WHERE team_id = t.id) as member_ids FROM "Teams" as t''')
            return cur.fetchall()
...
async def get_team(id: UUID) -> Team:
    with db.connection() as conn:
        with conn.cursor(row_factory=class_row(Team)) as cur:
            cur.execute('''SELECT t.id, t.name, ARRAY(SELECT user_id FROM "TeamMembers" WHERE team_id = t.id) as member_ids FROM "Teams" as t WHERE id = %s''', (id,))
...
async def create_team(team: TeamRequest) -> None:
    with db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute('''INSERT INTO "Teams" (name) VALUES (%s) RETURNING id''',
                              (team.name,))
            team_id = (cur.fetchone()).get("id", None)
            if team_id is None:
                raise HTTPException(status_code=500, detail="Failed to create team")
            
            cur.executemany('''INSERT INTO "TeamMembers" (team_id, user_id) VALUES (%s, %s)''', ((team_id, user_id) for user_id in team.member_ids))
...
async def add_member(team_id: UUID, user_id: UUID) -> None:
    with db.connection() as conn:
        with conn.cursor() as cur:
            cur.execute('''INSERT INTO "TeamMembers" (team_id, user_id) VALUES (%s, %s)''',
                              (team_id, user_id))
...
async def remove_member(team_id: UUID, user_id: UUID) -> None:
    with db.connection() as conn:
        with conn.cursor() as cur:
            cur.execute('''DELETE FROM "TeamMembers" WHERE team_id = %s AND user_id = %s''', (team_id, user_id))
