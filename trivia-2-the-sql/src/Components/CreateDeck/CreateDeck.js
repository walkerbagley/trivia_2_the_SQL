...
import { getQuestions, createDeck, updateDeck, getSortedQuestions } from '../../Services/Decks.js';
...
const CreateDeck = () => {
    ...
    const searchQuestions = async (cat, diff)  => {
      try {
        const ds = await getSortedQuestions(cat, diff);
        setQuestions(ds);
      } catch (error) {
        console.error("Failed to fetch decks:", error);
      }
    ...
    return (
        <div class='createpage'>
            ...
            <div class="desc_info">
              ...
              <button type="submit" className="create_button" onClick={() => createDeck(deckName, deckDesc)}>Create</button>
            </div>
            ...
            <div id='questionlist' style={{ height: '300px', overflow: 'scroll' }}>
              <div className="grid-container">
                ...
                allQuestions.map((question) => (
                  <li>{question.question} <button onClick={() => updateDeck(deckName, deckDesc)}>add</button></li>
              ))
              : <></>}
